...
set -e
cd "$(dirname "$0")/.."
# systemd EnvironmentFile로 안 들어왔으면 .env에서 OPINION_PROXY 읽기
if [ -z "$OPINION_PROXY" ] && [ -f .env ]; then
  OPINION_PROXY=$(grep -E '^OPINION_PROXY=' .env | head -1 | cut -d= -f2- | tr -d '"' | xargs) || true
fi
